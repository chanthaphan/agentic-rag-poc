...
- **Knowledge**: per-category files with PDF text status, drag-and-drop upload, per-file re-ingest, chunk browser (click a file), hybrid index search next to the knowledge-base retrieve, a built-in crawler (start URL + prefix + max pages, follows links and PDFs, no external service) and single-URL import.
