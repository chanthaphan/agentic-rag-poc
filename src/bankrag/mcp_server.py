"""An MCP server exposing the bank's live services as tools for external MCP clients.

The knowledge base answers from documents; this answers from the bank's live APIs, which is what a rate or a branch
needs. The app's own agents call the same tool bodies in-process (tools.py); this endpoint, mounted at
`/mcp/services`, is for any other MCP client. In Azure it stays behind the app's Easy Auth and the caller is pinned
to the object ids in `MCP_CALLER_PRINCIPALS`; `MCP_TOOL_KEY` is the local-dev guard, where there is no Easy Auth to
authenticate anyone.
...
from .tools import BRANCH_DESCRIPTION, FX_DESCRIPTION, TOOL_BRANCH, TOOL_FX, find_branch_impl, fx_rate_impl
...
def build_server(settings: Settings) -> MCPServer:
    ...
    @server.tool(name=TOOL_FX, description=FX_DESCRIPTION)
    ...
    @server.tool(name=TOOL_BRANCH, description=BRANCH_DESCRIPTION)
...
def build_asgi(settings: Settings) -> tuple[Callable, Any]:
    ...
    # to the container's public name. Allow exactly our own names (plus local dev / tests).
    #
...
def _reject(settings: Settings, headers: dict[str, str]) -> str:
    ...
    it to the allowed object ids, because Easy Auth alone would let any signed-in tenant user call the tool.
    MCP_TOOL_KEY stays as the local-dev guard, where there is no Easy Auth to authenticate anyone."""
