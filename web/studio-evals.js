...
function renderRun(run) {
  ...
  if (set === "compare") {
    ...
    const sumTable = `<div class="tw"><table class="list cmp-sum"><thead><tr><th>model</th><th>avg latency</th><th>total cost</th><th>errors</th></tr></thead><tbody>${models.map((m) => { const errs = rows.filter((r) => (r.by_model || {})[m]?.error).length; return `<tr><td><b>${esc(m)}</b></td><td>${fmtMsE(sm[`${m} avg_ms`] || 0)}</td><td>${fmtUsd(sm[`${m} cost_usd`])}</td><td>${errs || ""}</td></tr>`; }).join("")}</tbody></table></div>`;
