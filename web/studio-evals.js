...
const SET_LABEL = { routing: "Routing", rag: "Grounded answers", compare: "Model comparison" };
...
async function loadEvals() {
  ...
  await loadEvalRuns();
  for (const set of ["routing", "rag", "compare"]) if (!EV.results[set]) { const latest = EV.runs.find((r) => r.set === set); if (latest) showRun(latest.id, false); }
}
S.loaders.evals = loadEvals;
for (const set of ["routing", "rag"]) {
  $(`#ev-add-${set}`).addEventListener("click", () => { evState[set] = collect(set); evState[set].push(set === "routing" ? { q: "", skill: "" } : { q: "", skill: null, expect: [], require_source: true }); renderQuestions(set); $(`#ev-${set} tbody tr:last-child .q`)?.focus(); });
  ...
  $(`#ev-run-${set}`).addEventListener("click", async () => {
    ...
    try { await api(`/evals/${set}`, json(evState[set], "PUT")); const j = await api(`/evals/run?set=${set}`, { method: "POST" }); $(`#ev-status-${set}`).textContent = ""; watchEval(j.job_id, set); } catch (e) { $(`#ev-status-${set}`).textContent = e.message; }
  });
...
async function loadEvalRuns() {
  ...
  $("#ev-trend").innerHTML = trend("routing", "routing accuracy, last runs") + trend("rag", "grounded pass rate, last runs");
}
