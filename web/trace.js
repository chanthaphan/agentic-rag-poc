...
const TR = (() => {
  ...
  if (tr.handoff) html += `<div class="tc-row"><span class="k">handoff</span><span class="v">${esc(tr.handoff.concierge)} → ${esc(tr.handoff.specialist)} over A2A (${(tr.handoff.calls || []).length} call${(tr.handoff.calls || []).length === 1 ? "" : "s"})${tr.cost && tr.cost.note ? `<br><span class="muted">${esc(tr.cost.note)}</span>` : ""}</span></div>`;
