...
ENVVARS=(
  ...
  APP_USER_NAME="${APP_USER_NAME:-Pim}" APP_USER_INITIALS="${APP_USER_INITIALS:-PW}" ASSISTANT_GENDER="${ASSISTANT_GENDER:-male}" DATA_DIR=/data
  REALTIME_DEPLOYMENT="${REALTIME_DEPLOYMENT:-gpt-realtime-2.1}" REALTIME_VOICE="${REALTIME_VOICE:-cedar}" REALTIME_TRANSCRIBE_MODEL="${REALTIME_TRANSCRIBE_MODEL:-gpt-4o-mini-transcribe}" TTS_DEPLOYMENT="${TTS_DEPLOYMENT:-gpt-audio-1.5}"
)
if [ -n "${REALTIME_AVATAR_URL:-}" ]; then ENVVARS+=(REALTIME_AVATAR_URL="$REALTIME_AVATAR_URL"); fi
# the persona's name is whatever .env says; the image must not carry one of its own
if [ -n "${ASSISTANT_NAME:-}" ]; then ENVVARS+=(ASSISTANT_NAME="$ASSISTANT_NAME"); fi
if [ -n "${ASSISTANT_NAME_EN:-}" ]; then ENVVARS+=(ASSISTANT_NAME_EN="$ASSISTANT_NAME_EN"); fi
