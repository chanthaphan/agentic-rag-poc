...
az containerapp update -g "$RG" -n "$APP" --image "$IMAGE" --min-replicas "$MIN" --max-replicas 1 \
  --set-env-vars AZURE_TENANT_ID="$AZURE_TENANT_ID" AZURE_SUBSCRIPTION_ID="$AZURE_SUBSCRIPTION_ID" AZURE_RESOURCE_GROUP="$RG" \
    ...
    STUDIO_PASSWORD=secretref:studio-password STUDIO_ADMINS="${STUDIO_ADMINS:-}" STUDIO_TESTERS="${STUDIO_TESTERS:-}" APP_USER_NAME="${APP_USER_NAME:-Pim}" APP_USER_INITIALS="${APP_USER_INITIALS:-PW}" ASSISTANT_NAME="${ASSISTANT_NAME:-Assistant}" DATA_DIR=/data -o none
